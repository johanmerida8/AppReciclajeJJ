...
class RecyclingDataService {
  ...
  Future<List<RecyclingItem>> loadRecyclingItems() async {
    ...
    for (Article article in articles) {
      if (article.state == 1 && article.id != null && article.name != null) {
        try {
          final item = await _convertArticleToRecyclingItem(
            article,
            categories,
          );
          if (item != null) {
          ...
          if (workflowStatus == 'asignado' || workflowStatus == 'en_proceso') {
            ...
            if (request != null) {
              ...
              if (scheduledDay != null && scheduledEndTime != null) {
                try {
                  // Parse scheduled date and end time
                  ...
                  // If current time is past scheduled end time, mark as vencido
                  if (DateTime.now().isAfter(scheduledDateTime)) {
                    workflowStatus = 'vencido';
                    print(
                      '⏰ Task for article ${article.id} is overdue (vencido)',
                    );
                  }
                } catch (e) {
                  print(
                    'Error checking vencido status for article ${article.id}: $e',
                  );
                }
              }
            }
