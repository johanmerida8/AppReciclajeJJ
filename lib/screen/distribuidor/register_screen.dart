...
class _RegisterScreenState extends State<RegisterScreen> {
  ...
  void signUserUp() async {
    ...
    try {
      ...
      try {
        // Try to sign in with email and a random password to check if account exists
        // This is a workaround since Supabase doesn't provide direct API to check email existence
        final _ = await Supabase.instance.client.auth.signInWithOtp(
          email: email,
          shouldCreateUser: false, // Don't create user, just check
        );

        // If we get here without error, email might exist in auth
        // Let's check the users table
        final existingAuthUser =
        ...
        // Error with OTP means we can proceed - email doesn't exist yet
        print('✅ Email available for registration');
      }
      // --- Lógica para rol ---
      String role = 'distribuidor';
      if (names.toLowerCase() == 'admin' ||
          names.toLowerCase() == 'administrador') {
        ...
        if (existingAdmins.length >= 3) {
          ...
          ScaffoldMessenger.of(context).showSnackBar(
            const SnackBar(
              content: Text(
                'Ya existen 3 administradores registrados. No se pueden crear más.',
              ),
              backgroundColor: Colors.red,
            ),
          );
      ...
      // --- Comprobar email duplicado en tabla users (double-check) ---
      final existingUser =
          await Supabase.instance.client
              .from('users')
              .select('idUser')
      ...
      if (mounted) {
        ...
        ScaffoldMessenger.of(context).showSnackBar(
          SnackBar(
            content: Text('Cuenta creada exitosamente como $role'),
