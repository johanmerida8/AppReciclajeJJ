...
class _EditProfileScreenState extends State<EditProfileScreen> {
  ...
  Future<void> _uploadAvatar(XFile imageFile) async {
    ...
    try {
      ...
      try {
        ...
        await storage.from('article-images').list(
          path: '',
          searchOptions: const SearchOptions(limit: 1),
        );
        print('✅ Bucket article-images accesible');
      } catch (e) {
  ...
  Future<void> _saveProfile() async {
    ...
    try {
      // Extract file path and name from URL if avatar exists
      String? avatarFilePath;
      String? avatarFileName;
      
      if (_avatarUrl != null) {
        // Extract path from URL: users/{userId}/avatars/avatar_{timestamp}.jpg
        final uri = Uri.parse(_avatarUrl!);
        final pathSegments = uri.pathSegments;
        
        // Find the index of 'users' in the path
        final usersIndex = pathSegments.indexOf('users');
        if (usersIndex != -1 && pathSegments.length > usersIndex + 1) {
          // Reconstruct the path: users/{userId}/avatars/
          final userIdSegment = pathSegments[usersIndex + 1];
          avatarFilePath = 'users/$userIdSegment/avatars/';
          
          // Get the filename (last segment)
          avatarFileName = pathSegments.last;
        }
      }
      ...
      print('   Avatar Path: ${updatedUser.avatarFilePath}');
      print('   Avatar File: ${updatedUser.avatarFileName}');
      ...
      await Supabase.instance.client
          ...
          .update({
            ...
            'avatarFilePath': updatedUser.avatarFilePath,
