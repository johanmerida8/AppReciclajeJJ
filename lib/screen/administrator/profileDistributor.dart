...
class ProfileDistributorScreen extends StatefulWidget {
  ...
  const ProfileDistributorScreen({super.key, required this.userId});

  @override
  State<ProfileDistributorScreen> createState() =>
...
class _ProfileDistributorScreenState extends State<ProfileDistributorScreen> {
  ...
  Future<void> _loadUserData() async {
    try {
      currentUser = await usersDatabase.getUserById(widget.userId);
      ...
      for (final article in userArticles) {
        if (article.id != null) {
          final pattern = 'articles/${article.id}';
          final photo = await mediaDatabase.getMainPhotoByPattern(pattern);
  ...
  // Mostrar historial del artículo en un BottomSheet
  void _showArticleHistory(Article article) {
    showModalBottomSheet(
      context: context,
      isScrollControlled: true, // para que ocupe más altura
      backgroundColor: Colors.transparent,
      builder:
          (context) => HistoryBottomSheet(
        ...
        delegate: SliverChildBuilderDelegate(
          (context, index) {
            ...
            return GestureDetector(
              onLongPress: () => _showArticleHistory(article),
